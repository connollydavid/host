...
```
https://github.com/connollydavid/template-agentic-host
```
...
5. Re-stamp `.agentic-host` to the target revision; record it in `call/` and
   `MEMORY.md`.
...
- **Repo-root config the tools read.** `.host-software` (the *Where* recipe),
  `.agentic-host` (the stamp), `.host-remap` (the rename dictionary, transient),
